...
describe("CLI media-plan approval", () => {
  test("requires --approve to match the plan and atomically transitions it", async () => {
    ...
    await expect(approvePlanForSubmission(db, { planId: "plan-1" })).rejects.toThrow("--approve <planId>");
    await expect(approvePlanForSubmission(db, { planId: "plan-1", approve: "plan-2" })).rejects.toThrow("matching --planId");
    ...
    await expect(approvePlanForSubmission(db, { planId: "plan-1", approve: "plan-1" })).rejects.toThrow("could not be approved");
  });
});
