...
const plan: PreparedGenerationPlan = {
  ...
  items: [{
    ...
    effectiveSettings: { prompt: "A red panda", model: "nano-banana-2-lite", resolution: "1K" },
    price: { status: "exact", credits: 4, rateCardVersion: "2026-08-17", verifiedAt: "2026-08-17", sourceUrl: "https://example.com", sourceFingerprint: "test" },
  }],
...
describe("MCP media-plan approval", () => {
  test("requires accepted form confirmation", async () => {
    const elicitInput = jest.fn(async () => ({ action: "accept" as const, content: { confirm: true } }));
    const server = {
      getClientCapabilities: () => ({ elicitation: { form: {} } }),
      elicitInput,
    } as unknown as Pick<Server, "getClientCapabilities" | "elicitInput">;

    await expect(requestMcpPlanApproval(server, plan)).resolves.toEqual({ approved: true, reason: "Host confirmed the plan." });
  ...
  test("supports legacy empty elicitation capabilities as form-capable", async () => {
    ...
    await expect(requestMcpPlanApproval(server, plan)).resolves.toEqual({ approved: true, reason: "Host confirmed the plan." });
    expect(elicitInput).toHaveBeenCalledWith(expect.objectContaining({ mode: "form" }));
  });
  ...
  test("decline and unsupported elicitation never approve", async () => {
    ...
    await expect(requestMcpPlanApproval(declined, plan)).resolves.toMatchObject({ approved: false });
    await expect(requestMcpPlanApproval(unsupported, plan)).resolves.toMatchObject({ approved: false, reason: expect.stringContaining("does not support") });
