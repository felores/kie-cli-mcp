...
function resultPayload(result: { content: Array<{ type: string; text: string }> }): Record<string, unknown> {
...
describe("MCP media planning integration", () => {
  test("exposes documentation only for callable tools by default", async () => {
    ...
    try {
      ...
      await expect(client.readResource({ uri: "kie://tools/nano_banana_image" }))
        .rejects.toThrow("Resource not found");
      await expect(client.readResource({ uri: "kie://tools/get_task_status" }))
        .resolves.toMatchObject({
          contents: [expect.objectContaining({ uri: "kie://tools/get_task_status" })],
        });
    } finally {
      await client.close();
      await server.close();
      ...
      if (previousDirectGeneration === undefined) delete process.env.KIE_AI_ALLOW_DIRECT_GENERATION;
      else process.env.KIE_AI_ALLOW_DIRECT_GENERATION = previousDirectGeneration;
      rmSync(databaseDirectory, { recursive: true, force: true });
    }
  });

  test("elicits approval through the transport and submits only the confirmed plan", async () => {
    const databaseDirectory = mkdtempSync(join(tmpdir(), "kie-mcp-media-plan-"));
    const previousApiKey = process.env.KIE_AI_API_KEY;
    const previousDatabasePath = process.env.KIE_AI_DB_PATH;
    process.env.KIE_AI_API_KEY = "test-key-no-network";
    process.env.KIE_AI_DB_PATH = join(databaseDirectory, "tasks.db");
    ...
    (app as unknown as { client: { generateNanoBananaImage: typeof providerCall } })
    ...
    const elicitationRequests: Array<{ message: string; confirmRequired: boolean }> = [];
    client.setRequestHandler(ElicitRequestSchema, async (request) => {
      if (request.params.mode !== "form") throw new Error("Expected form elicitation.");
      elicitationRequests.push({
        message: request.params.message,
        confirmRequired: request.params.requestedSchema.required?.includes("confirm") ?? false,
      });
    ...
    try {
      ...
      const unconfirmed = resultPayload(await client.callTool({
        name: "prepare_media_generation",
        arguments: {
          items: [{ tool: "nano_banana_image", args: { prompt: "A red panda" } }],
        },
      }) as { content: Array<{ type: string; text: string }> });
      expect(unconfirmed).toMatchObject({ success: true, approved: false, status: "prepared" });
      expect(providerCall).not.toHaveBeenCalled();

      const blockedSubmission = resultPayload(await client.callTool({
        name: "submit_media_generation",
        arguments: { planId: unconfirmed.planId },
      }) as { content: Array<{ type: string; text: string }> });
      expect(blockedSubmission).toMatchObject({ success: false, error: expect.stringContaining("not approved") });
      expect(providerCall).not.toHaveBeenCalled();

      confirmsPlan = true;
      const approved = resultPayload(await client.callTool({
        name: "prepare_media_generation",
        arguments: {
          items: [{ tool: "nano_banana_image", args: { prompt: "A confirmed red panda" } }],
        },
      }) as { content: Array<{ type: string; text: string }> });
      expect(approved).toMatchObject({ success: true, approved: true, status: "approved" });
      expect(providerCall).not.toHaveBeenCalled();
      expect(elicitationRequests).toEqual(expect.arrayContaining([
        expect.objectContaining({ confirmRequired: true, message: expect.stringContaining("Approve media generation plan") }),
      ]));

      const submitted = resultPayload(await client.callTool({
        name: "submit_media_generation",
        arguments: { planId: approved.planId },
      }) as { content: Array<{ type: string; text: string }> });
      expect(submitted).toMatchObject({ success: true, planId: approved.planId });
