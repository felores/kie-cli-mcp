...
const generationTools = TOOL_REGISTRY.filter((tool) => tool.category !== "utility");
...
describe("MCP direct generation access", () => {
  test("does not list or allow direct generation by default while utilities remain callable", () => {
    const listedTools = TOOL_REGISTRY
      .filter((tool) => isMcpToolCallable(tool, enabledTools, false))
      .map((tool) => tool.name);

    expect(generationTools.every((tool) => !listedTools.includes(tool.name))).toBe(true);
    expect(generationTools.every((tool) => !isMcpToolCallable(tool, enabledTools, false))).toBe(true);
  ...
  test("allows direct generation only with the explicit compatibility opt-in", () => {
    expect(generationTools.every((tool) => isMcpToolCallable(tool, enabledTools, true))).toBe(true);
    ...
    try {
      ...
      if (previous === undefined) delete process.env.KIE_AI_ALLOW_DIRECT_GENERATION;
