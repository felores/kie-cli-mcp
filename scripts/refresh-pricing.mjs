...
function requireHttpsUrl(value, field, index) {
  ...
  try {
    ...
    throw new Error(
      `Formula proposal ${index + 1} requires an HTTPS ${field}.`,
    );
  }
  return text;
}
...
function validateTestReference(testPath, repositoryRoot, index) {
  ...
  if (!/\.(test|spec)\.[cm]?[jt]sx?$/.test(text)) {
    throw new Error(
      `Formula proposal ${index + 1} test reference must target a test file.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Formula proposal ${index + 1} references a missing repository test: ${text}.`,
    );
  }
  return text;
}
...
function validateExactCreditsFormula(value, index) {
  ...
  if (
    !/^\d+(?:\.\d+)?\s+credits?\s+per\s+(?:\d+\s+)?(?:images?|seconds?|minutes?|requests?|videos?|audio|characters?|tokens?)$/i.test(
      formula,
    )
  ) {
    throw new Error(
...
export function parseRefreshArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    throw new Error(
      `Unknown pricing refresh argument: ${argument}. ${usage()}`,
    );
  }
  if (proposalsPath && !apply) {
    throw new Error(
      "--proposals requires --apply because refresh is read-only by default.",
    );
  }
  return { help: false, apply, proposalsPath };
}

export function validateFormulaProposals(
  proposals,
  repositoryRoot = REPOSITORY_ROOT,
) {
  if (!Array.isArray(proposals))
    throw new Error("proposedFormulaUpdates must be an array.");
  return proposals.map((proposal, index) => {
    ...
    if (proposal.kind !== "exact-credits") {
      throw new Error(
        `Formula proposal ${index + 1} must declare kind exact-credits.`,
      );
    }
    const formula = validateExactCreditsFormula(proposal.formula, index);
    if (!isIsoDate(proposal.verifiedAt)) {
      throw new Error(
        `Formula proposal ${index + 1} requires an ISO verification date.`,
      );
    }
    if (!Array.isArray(proposal.tests) || proposal.tests.length === 0) {
      throw new Error(
        `Formula proposal ${index + 1} requires one or more tests.`,
      );
    }
    return {
      kind: proposal.kind,
      ...
      tests: proposal.tests.map((testPath) =>
        validateTestReference(testPath, repositoryRoot, index),
      ),
    };
  });
}
...
function sourceFreshness(sources, now) {
  ...
  return sources.map((source) => ({
    ...source,
    freshness:
      isIsoDate(source.verifiedAt) && new Date(source.verifiedAt) >= staleAfter
        ? "fresh"
        : "stale",
  }));
}
...
export function refreshPricing({
  ...
  if (!proposalsPath) {
    return {
      ...
      noOp: true,
      message:
        "No eligible proposed formula updates were supplied. No files were changed.",
    };
  }

  const candidate = readJson(resolve(proposalsPath));
  const proposals = validateFormulaProposals(
    candidate.proposedFormulaUpdates,
    repositoryRoot,
  );
  ...
  return {
    ...
    message:
      "Validated formula proposals were recorded in the evidence manifest. Rate-card code was not changed.",
  };
}
