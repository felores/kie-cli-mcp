...
- OAuth for public remote deployments is designed and documented but deferred by design
- Fixed the MCP `start`/`dev` scripts' `.env` path: the env file lives at the repository root, so the scripts now load `../../.env` from the package directory; the Inspector instructions in `AGENTS.md` run from the repo root. (`docs/OAUTH.md`): it activates only with a public remote deployment plus per-owner tenant state. The current local/ self-hosted bearer model is unchanged.
