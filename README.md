...
- **OpenAI transport**: `@felores/kie-ai-openai-server`, a loopback HTTP server that exposes selected image/video models through OpenAI-shaped routes. Version 0.2 adds model discovery and native Infinite Canvas image-format and video-preset compatibility. See [`docs/openai-transport.md`](docs/openai-transport.md).
